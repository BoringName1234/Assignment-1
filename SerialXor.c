...
// ******************************************
// This defines the number used in the Xor shift algorithm
 ...
 */
void attemptInfection(int **int_grids, float **float_grids, int *dims, int r, int source_position, struct xorshift32_state *xor_num_pointer){                        
    ...
    */
    ...
    for(int i = 0; i < total_columns * total_rows; i++){
        if(manhattan_table[i] <= r){
            xorshift32(xor_num_pointer);
            uint32_t random = xor_num_pointer->a;
            float random_number = (float)random/(float)(UINT32_MAX);
 ...
 */
int simulate_step(int r, int rec_time, int *dims, int **int_grids, float **float_grids, struct xorshift32_state *xor_num_pointer){
    ...
    for(int i = 0; i < num_elements; i++){
        if(grid[i] == 1){    
            ...
            attemptInfection(int_grids, float_grids, dims, r, i, xor_num_pointer);
        }
    }
    //printf("3");
 ...
 */
int single_simulation(int r, int rec_time, int *dims, int **int_grids, float **float_grids, struct xorshift32_state *xor_num_pointer){
    ...
    int infections = 1;
    while(infections == 1){
        infections = simulate_step(r, rec_time, dims, int_grids, float_grids, xor_num_pointer);
...
int main (int argc, char *argv[])
{
    ...
    struct xorshift32_state xor_num;
    struct xorshift32_state *xor_num_pointer = &xor_num;

    gettimeofday(&start_time, NULL);
    xor_num_pointer->a = start_time.tv_sec;
    xorshift32(xor_num_pointer);
    int result = xor_num_pointer->a;
    ...
    for(int i = 0; i < max_runs; i++){
        num_infections += single_simulation(r, rec_time, dims, int_grids, float_grids, xor_num_pointer);
